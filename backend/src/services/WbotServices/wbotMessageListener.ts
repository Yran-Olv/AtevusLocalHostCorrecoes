...
// Configurar caminho do FFmpeg para fluent-ffmpeg
...
const getSenderMessage = (
  ...
  const participantPn = (msg.key as any).participantPn;
  if (participantPn) {
    console.log(`✅ Usando participantPn (número real em grupo): ${participantPn}`);
    return jidNormalizedUser(participantPn);
  }
...
export const verifyTypeMessage = (msg: string): "group" | "lid" | "jid" | "unknown" => {
  ...
  if (msg.endsWith("@g.us")) return "group";
  if (msg.endsWith("@lid")) return "lid";
  if (msg.endsWith("@s.whatsapp.net")) return "jid";
  return "unknown";
};
...
const getContactMessage = async (msg: WAMessage, wbot: Session) => {
  ...
  const senderJid = verifyTypeMessage(msg.key.remoteJid) === 'jid'
    ? msg?.key?.remoteJid
    : verifyTypeMessage(msg.key.remoteJid) === 'lid'
      ? msg.key.senderPn || null : null;

  const senderLid = verifyTypeMessage(msg.key.remoteJid) === 'lid'
    ? msg.key.senderLid || msg?.key?.remoteJid
    : verifyTypeMessage(msg.key.remoteJid) === 'jid'
      ? msg.key.senderLid || null : null;
...
export const verifyTypeMessage_ = (msg: string): "group" | "lid" | "jid" | "unknown" => {
...
//   // Detectar LID e extrair para salvar separadamente
//   const isLidContact = msgContact.id.includes("@lid");
//   const lid = isLidContact ? msgContact.id : (msgContact as any).lid || undefined;
...
const verifyContact = async (msgContact: IMe, wbot: Session, companyId: number): Promise<Contact> => {
  ...
  try {
    ...
    Sentry.captureException(e);//coloca cache aqui
    profilePicUrl = `${process.env.FRONTEND_URL}/nopicture.png`;
  }

  const isGroup = msgContact.id.includes("g.us");
  const jidNumber = msgContact.id.replace(/\D/g, "");
  const name = msgContact?.name || jidNumber;


  if (isGroup) {

    // console.log("Verificando contato de grupo:", msgContact);
    ...
    const contactData = {
      ...
      isGroup: true,
      companyId,
      whatsappLid: msgContact.senderLid || msgContact.id,
      whatsappId: wbot.id,
    };
  ...
  const sender_lid = msgContact?.senderLid;
  const sender_jid = msgContact?.senderPn;

  /**
   * pega o senderLid e verifica se ele é um numero lid
   * se for um numero lid verificar se veio cum numero jid junto para poder atualizar esse lid para jid
   * so vai tentar normalizar o contato se ele tiver os 2 numeros JID e LID
   */

  // so vai fazer a normalização se tiver os 2 numeros JID e LID
  if (sender_lid && sender_jid) {

    /**NORMALIZA SE TIVER  2 CONTATOS COM 1 COM JID E OUTRO COM LID */
    const contactLid = await Contact.findOne({ where: { number: sender_lid, companyId } });
    const contactJid = await Contact.findOne({ where: { number: sender_jid.replace(/\D/g, ""), companyId } });

    if (contactLid && contactJid) {

      logger.info(`Normalizando contatos jid e lid: ${sender_jid} / ${sender_lid} na empresa ${companyId} `);

      /**se eu tenho os 2 contatos ja criados , vou copiar os dados do lid para o jid e apagar o contato lid */

      await contactJid.update({ whatsappLid: sender_lid, profilePicUrl });
      await contactJid.reload();

      const ticketLid = await Ticket.findOne({ where: { contactId: contactLid.id } });
      const ticketJid = await Ticket.findOne({ where: { contactId: contactJid.id } });

      if (ticketLid && ticketJid) {

        const messagesLid = await Message.findAll({
          where: { ticketId: ticketLid.id, companyId }
        });

        await Promise.all(
          messagesLid.map(msg =>
            msg.update({
              contactId: contactJid.id,
              ticketId: ticketJid.id
            })
          )
        );

        await contactLid.destroy();

        const io = getIO();
        io.to("open")
          .to("pending")
          .to(ticketLid.id.toString())
          .emit(`company-${ticketLid.companyId}-ticket`, {
            action: "delete",
            ticketId: ticketLid.id
          });
  ...
  /** NORMALIZA SE TIVER 2 CONTATOS COM O MESMO NUMERO JID ( 2 CONTATOS MAPEADOS )*/
  const contactsWhitSameNumber = await Contact.findAll({
    where: {
      companyId,
      number: sender_jid.replace(/\D/g, "")
    }
  });

  if (contactsWhitSameNumber.length >= 2) {

    logger.info(`Normalizando contatos com mesmo numero jid: ${sender_jid} na empresa ${companyId} `);

    const ticketsWhitSameNumber = await Ticket.findAll({
      where: {
        companyId,
        contactId: contactsWhitSameNumber.map(c => c.id)
      }
    });

    const sorted = ticketsWhitSameNumber.sort((a, b) => a.id - b.id);

    const menorTicketId = sorted[0];                      // ticket com menor ID
    const maiorTicketId = sorted[sorted.length - 1];      // ticket com maior ID

    const lastContact = await Contact.findByPk(maiorTicketId.contactId);

    const mensagens = await Message.findAll({
      where: {
        ticketId: maiorTicketId.id,
        companyId
      }
    });
    ...
    await Promise.all(
      mensagens.map(msg =>
        msg.update({
          contactId: menorTicketId.contactId,
          ticketId: menorTicketId.id
        })
      )
    );

    await lastContact.destroy();

    const io = getIO();
    io.to("open")
      .to("pending")
      .to(maiorTicketId.id.toString())
      .emit(`company-${maiorTicketId.companyId}-ticket`, {
        action: "delete",
        ticketId: maiorTicketId.id
      });
  ...
  const contactData = {
    name,
    number: msgContact.senderPn || msgContact.senderLid || msgContact.id,
    whatsappLid: msgContact.senderLid || msgContact.id || null,
    profilePicUrl,
    isGroup: false,
    companyId,
  };
        ...
        if (fileExists) {
          ...
          const debouncedSentgreetingMediaAttachment = debounce(
            async () => {
              ...
              const sentMessage = await wbot.sendMessage(GetJidOf(ticket), { ...optionsMsg });
        ...
        if (fileExists) {
          ...
          const debouncedSentgreetingMediaAttachment = debounce(
            async () => {
              ...
              let sentMessage = await wbot.sendMessage(GetJidOf(ticket), { ...optionsMsg });
...
const convertWavToAnotherFormat = (
  ...
  return new Promise((resolve, reject) => {
    ...
    // Configuração específica para Opus (melhor para WhatsApp no Android)
    if (toFormat === 'opus') {
      command
        .audioCodec('libopus')
        .audioBitrate('128k')
  ...
  if (msg.message?.conversation || msg.message?.extendedTextMessage?.text) {
    ...
    if (prompt.voice === "texto") {
      ...
      convertTextToSpeechAndSaveToFile(
        ...
        try {
          const sendMessage = await wbot.sendMessage(GetJidOf(ticket), {
            audio: { url: `${publicFolder}/${fileNameWithOutExtension}.opus` },
            mimetype: "audio/ogg; codecs=opus",
            ptt: true
          });
          await verifyMediaMessage(sendMessage!, ticket, contact, ticketTraking, false, false, wbot);
          deleteFileSync(`${publicFolder}/${fileNameWithOutExtension}.opus`);
...
const handleMessage = async (
  ...
  try {
    ...
    if (isGroup) {
      ...
      const hasLidInRemoteJid = isLidUser(msg.key.remoteJid);

      if (hasLidInRemoteJid && grupoMeta.participants) {
        console.log('\n' + '='.repeat(80));
        console.log('🔍 GRUPO COM LID - Analisando Participantes');
        console.log('='.repeat(80));
        console.log('Grupo ID:', grupoMeta.id);
        console.log('Total de participantes:', grupoMeta.participants.length);
        console.log('\nPrimeiros 5 participantes:');
        grupoMeta.participants.slice(0, 5).forEach((p: any, idx: number) => {
          console.log(`  ${idx + 1}. ID: ${p.id} | Admin: ${p.admin || 'null'}`);
        });

        const msgNumber = msg.key.remoteJid.replace('@lid', '').split(':')[0];
        console.log('\n🔍 Buscar por padrão:', msgNumber);
        const matchingByPattern = grupoMeta.participants.find((p: any) =>
          p.id.includes(msgNumber) || p.id.startsWith(msgNumber)
        );
        if (matchingByPattern) {
          console.log('✅ PARTICIPANTE (padrão):', matchingByPattern.id);
        } else {
          console.log('❌ Não encontrado por padrão');
        }

        console.log('\n🔍 Comparar com areJidsSameUser():');
        const matchingBySameUser = grupoMeta.participants.find((p: any) => {
          try {
            return areJidsSameUser(msg.key.remoteJid, p.id);
          } catch (e) {
            return false;
          }
        });
        if (matchingBySameUser) {
          console.log('✅ MESMO USUÁRIO:', matchingBySameUser.id);
        } else {
          console.log('❌ areJidsSameUser não encontrou');
        }
      ...
      if (!msg.key.fromMe) {
        ...
        if (!isNil(whatsapp.collectiveVacationMessage && !isGroup)) {
          ...
          if (currentDate.isBetween(moment(whatsapp.collectiveVacationStart), moment(whatsapp.collectiveVacationEnd))) {
            ...
            wbot.sendMessage(GetJidOf(ticket), { text: whatsapp.collectiveVacationMessage })
...
const wbotMessageListener = (wbot: Session, companyId: number): void => {
  ...
  wbot.ev.on('lid-mapping.update' as any, (mapping: any) => {
    console.log('\n' + '='.repeat(80));
    console.log('🔍 LID MAPPING UPDATE');
    console.log('='.repeat(80));
    console.log('Mapeamento:', JSON.stringify(mapping, null, 2));
    console.log('='.repeat(80) + '\n');
  });
};
