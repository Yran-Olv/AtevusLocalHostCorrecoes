...
export const importXls = async (req: Request, res: Response): Promise<Response> => {
  ...
  const contactData = {
    name: `${name}`,
    number: validNumber.jid || validNumber.lid,
    profilePicUrl: "",
    isGroup: false,
    email,
    companyId,
...
export const update = async (
  ...
  if (oldContact.number != contactData.number && oldContact.channel == "whatsapp") {
    ...
    const number = validNumber.jid || validNumber.lid;
...
export const getContactProfileURL = async (req: Request, res: Response) => {
  ...
  if (number) {
    ...
    const profilePicUrl = await GetProfilePicUrl(numberIsValided, companyId);

    const contact = await NumberSimpleListService({ number: numberIsValided, companyId: companyId })
